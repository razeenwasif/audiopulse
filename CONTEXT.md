...
- **AI agent — use-case (1) DONE; (2)/(3) backlog.** Local LLM via Ollama (`:11434`, plain `net/http`, no SDK, **no fine-tuning**). **(1) Natural-language control** ("play X", "shuffle on", "skip", "loop this", "pause", "volume 30") is **shipped** — `internal/agent` + `:` prompt, JSON intent (`format:"json"` — Gemma's tool-calling is shaky on small variants), mapped to existing commands (ADR-0013). Still wanted: (2) **recommender/DJ** → RAG over the (soon local) library metadata picks tracks; (3) **chat/Q&A** sidebar. Both extend the same schema/agent (add actions; for (2) add a retrieval/context step) and pair well with Phase B (offline, no API). Fine-tune only if a repeatable failure can't be fixed with prompt/schema/few-shot.
...
- `internal/agent/agent.go` — Ollama client + `Interpret` + JSON parse/normalize/model-pick (pure logic tested in `agent_test.go`).
