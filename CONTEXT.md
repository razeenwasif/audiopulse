...
- **Export hardening — DONE.** `internal/downloader` now writes `_export.log` (raw spotDL output) + `_export-failures.txt` (failed track names via `failureName`/`writeFailures`) into the music dir, runs smaller batches (40) each guarded by a **stall `watchdog`** (kills a batch silent for 3 min → retried next run), and the done overlay points at the failures file. Diagnosed reality: the high failure rate is **genuinely-unavailable content** (Spotify Singles/Sessions/Live exclusives + odd/unicode-titled tracks not on YouTube), **not** throttling or a stale yt-dlp (2026.03.17, current). spotDL names files from Spotify metadata, audio from YouTube.
