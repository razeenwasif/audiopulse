...
- **AI agent — use-case (1) DONE; (2)/(3) backlog.** Local LLM via Ollama (`:11434`, plain `net/http`, no SDK, **no fine-tuning**). **(1) Natural-language control** ("play X", "shuffle on", "skip", "loop this", "pause", "volume 30") is **shipped** — `internal/agent` + `:` prompt, JSON intent (`format:"json"` — Gemma's tool-calling is shaky on small variants), mapped to existing commands (ADR-0013). **(2) recommender/DJ DONE** (ADR-0015): RAG over the library (local embeddings) + LLM discovery, resolved via Search. Still wanted: **(3) chat/Q&A** (`ActionAsk` stubbed) → Phase B multi-turn panel grounded in `internal/library` (`Search`+`Filter`+`Sample` already there). Fine-tune only if a repeatable failure can't be fixed with prompt/schema/few-shot.
